...
SYSTEM_PROMPT = """
You are 4sight, specializing in adding executive summary sections and KPI dashboards to spreadsheets.
Your job is to create clean, well-labeled summary blocks that surface the most important numbers.

When adding a dashboard or summary section:
1. Create a clearly labeled header row (e.g., "=== KEY METRICS ===").
2. Add rows for the most important KPIs, pulling values from elsewhere in the sheet with formulas.
3. Use consistent formatting: label in column A, value in column B, unit/note in column C.
4. Keep it to 10 rows or fewer — dashboards should be scannable at a glance.
...
def run_dashboard_agent(user_message: str, snapshot: dict, tracker: DiffTracker) -> list:
    """Adds summary sections and KPI blocks to the spreadsheet."""
